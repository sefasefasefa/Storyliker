...
## Auto-registration can race your manual createArtifact call
After moving `artifacts/<slug>` to `artifacts/<slug>-src` (per the fix sequence above), the platform may auto-detect the moved dir's existing `.replit-artifact/artifact.toml` and register it itself — using the `id` already in that toml — before or instead of your `createArtifact()` call. Symptom: `listArtifacts()` still returns empty right after the move, then after you call `createArtifact()` for a *different* slug, automatic_updates show artifacts registered for ALL the moved `-src` dirs at once (not just the one you called). If you already ran `createArtifact()` for one of them, you'll end up with a duplicate/wrong-type artifact (e.g. a fresh react-vite scaffold) alongside the auto-registered real one — delete the wrong one's directory (no `deleteArtifact` callback exists; removing the dir is enough, the platform reconciles automatic_updates showing it removed). Prefer: move dirs to `-src` first, wait/check `listArtifacts()` again before calling `createArtifact()` on any of them — auto-registration may already have handled it.

## Fix stale production paths after the `-src` rename
The pre-existing `artifact.toml` still references the pre-rename directory in production paths — `services.production.publicDir` (web) and `services.production.run.args` (api, e.g. `artifacts/<slug>/dist/index.mjs`). These must be updated to the new `-src` path via `verifyAndReplaceArtifactToml`, or production serving breaks even though dev workflows run fine. A code-review subagent pass after registration catches this reliably.
